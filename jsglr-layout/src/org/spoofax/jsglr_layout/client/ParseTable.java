...
public class ParseTable implements Serializable {
  ...
  /**
   * Number of possible characters to expect (0x10FFFF would be all chars of
   * UTF-8, but is not yet supported by the parse table format.)
   */
  ...
  // TODO: allocate prototypes to avoid measurable GC overhead in ParseTable
  // construction
  ...
  private boolean isInjection(IStrategoNamed prod) {
    ...
    // TODO: optimize - use constants for these constructors (a la
    // parseproductionreader)
  ...
  private ProductionAttributes parseProductionAttributes(IStrategoAppl attr) throws InvalidParseTableException {
    if (attr.getName().equals("attrs")) {
        ...
        if (ctor.equals("reject")) {
          ...
          if (ctor.equals("assoc")) {
            ...
            if (a.getName().equals("left") || a.getName().equals("assoc")) {
              ...
              // the current SDF manual and some tests seem to indicate that
              // non-assoc
            ...
            // Term needs to be shaped as term(cons(Constructor)) to be a
            // constructor
            if (t.getSubterm(0) instanceof IStrategoNamed) {
              ...
              } else if (child.getSubtermCount() == 0
                  && (child.getName().equals("ignore-layout") || child.getName().equals("ignore-indent"))) {
                isIgnoreLayout = true;
            ...
            // TODO Support other terms that are not a constructor (custom
            // annotations)
      ...
      return new ProductionAttributes(term, type, isRecover, isIgnoreLayout, layoutConstraintSource, layoutConstraint, isNewlineEnforced,
          isLongestMatch);
  ...
  // private int[] parseProductionLabels(IStrategoList ranges) throws
